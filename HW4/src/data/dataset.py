...
class SpeechDataset(Dataset):
    def __init__(self, lang, mode):
        ...
        features_path = (C.DATA_PATH+"%s.npy") % (self.mode)
        self.feats = np.load(features_path)

        self.trans = []
        if self.mode is not 'test':
            trans_tokenized = U.tokenizeTranscripts(self.mode)
        ...
        print("%s Dataset Loading Completed" % (self.mode))
    ...
    def __getitem__(self, index):
        if self.mode is not 'test':
            ...
            trans = np.asarray([C.SOS_TOKEN_IDX])
